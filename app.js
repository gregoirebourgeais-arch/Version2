...
function loadState() {
  try {
    ...
    state = { ...base, ...parsed };
    state.planning = { ...basePlanning, ...(state.planning || {}) };
    ...
    if (!Array.isArray(state.excelRecords)) state.excelRecords = [];
    if (!Array.isArray(state.excelFiles)) state.excelFiles = [];
    ...
    if (!Array.isArray(state.planning.cadences)) state.planning.cadences = [];
    ...
    persistPlanningSnapshots(state.planning.savedPlans);
  } catch (e) {
...
function readPlanningSnapshotsFromStorage() {
  try {
    ...
    return Array.isArray(parsed) ? parsed.filter(p => p && p.week) : [];
  } catch (e) {
...
function persistPlanningSnapshots(snaps) {
  const safe = Array.isArray(snaps) ? snaps.filter(s => s && s.week) : [];
  planningSnapshotCache = [...safe];
  planningSnapshots = [...safe];
  if (!state.planning) state.planning = { savedPlans: [] };
  state.planning.savedPlans = [...safe];
}

function savePlanningSnapshots(snaps) {
  persistPlanningSnapshots(snaps);
  try {
    localStorage.setItem(PLANNING_SNAPSHOTS_KEY, JSON.stringify(planningSnapshots));
  } catch (e) {
...
function getSavedPlanningList(forceReload = false) {
  if (forceReload) {
    ...
    if (stored.length) {
      savePlanningSnapshots(stored);
      return [...planningSnapshots];
    }
  }

  if (!planningSnapshots.length && Array.isArray(state?.planning?.savedPlans)) {
    persistPlanningSnapshots(state.planning.savedPlans);
  }
...
function getMergedPlanningSnapshots() {
  ...
  [
    ...
    ...(Array.isArray(planningSnapshotCache) ? planningSnapshotCache : []),
  ]
    .filter(Boolean)
    .forEach(snap => {
...
function savePlanningSnapshot() {
  ...
  persistPlanningSnapshots(merged);
