...
function getSavedPlanningList(forceReload = false) {
  ...
  if (forceReload) {
    ...
    if (stored.length) {
      persistPlanningSnapshots(stored);
      return [...stored];
    }
  }

  if (state.planning.savedPlans.length) return [...state.planning.savedPlans];
...
function loadPlanningSnapshots() {
  ...
  const merged = [...state.planning.savedPlans];
  stored.forEach(snap => {
    if (!snap || !snap.week) return;
    if (!merged.some(m => `${m.week}` === `${snap.week}`)) merged.push(snap);
  });
...
function savePlanningSnapshot() {
  ...
  const stored = getSavedPlanningList(true);
  const merged = [...stored];
